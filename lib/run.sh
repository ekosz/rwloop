...
run_iteration() {
  ...
  # Set HOME for credentials (Claude finds CLAUDE.md in cwd)
...
cmd_status() {
  ...
  # Session info
  local config
  config=$(cat "$session_dir/session.json" 2>/dev/null || echo "{}")
...
Generated by rwloop"
  ...
  # Create PR
  if [[ -n "$sprite_id" ]]; then
    # Use base64 to safely pass title and body with special characters
    local encoded_title encoded_body
    encoded_title=$(echo "$pr_title" | base64)
    encoded_body=$(echo "$pr_body" | base64)
    sprite exec -s "$sprite_id" -- sh -c "cd $SPRITE_REPO_DIR && gh pr create --title \"\$(echo '$encoded_title' | base64 -d)\" --body \"\$(echo '$encoded_body' | base64 -d)\"" 2>&1 || {
